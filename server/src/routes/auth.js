...
function makeToken(userId) {
  // simple demo token (not secure): "user:<id>"
  return `user:${userId}`;
}
...
router.post("/login", async (req, res) => {
  ...
  try {
    const { rows } = await pgPool.query("SELECT id, email, name, password_hash FROM users WHERE email=$1", [email]);
    const user = rows?.[0];
    if (!user || user.password_hash !== String(password)) return res.status(401).json({ message: "invalid credentials" });
    const token = makeToken(user.id);
...
router.get("/me", async (req, res) => {
  ...
  try {
    const { rows } = await pgPool.query("SELECT id, email, name FROM users WHERE id=$1", [userId]);
    const user = rows?.[0];
